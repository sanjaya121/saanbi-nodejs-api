...
import { CATEGORYQUERY } from '../query/categories.query';
// import { QUERY } from '../query/patient.query';
// CATEGORYQUERY
type ResultSet = [RowDataPacket[] | RowDataPacket[][] | OkPacket | OkPacket[] | ResultSetHeader, FieldPacket[]];

export const getCategoreis = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
  console.info(`[${new Date().toLocaleString()}] Incoming ${req.method}${req.originalUrl} Request from ${req.rawHeaders[0]} ${req.rawHeaders[1]}`);
  try {
    const pool = await connection();
    const result: ResultSet = await pool.query(CATEGORYQUERY.SELECT_PATIENTS);
...
//   try {
//     const pool = await connection();
//     const result: ResultSet = await pool.query(CATEGORYQUERY.SELECT_PATIENT, [req.params.patientId]);
...
//   try {
//     const pool = await connection();
//     const result: ResultSet = await pool.query(CATEGORYQUERY.CREATE_PATIENT, Object.values(patient));
...
//     const result: ResultSet = await pool.query(CATEGORYQUERY.SELECT_PATIENT, [req.params.patientId]);
//     if (((result[0]) as Array<any>).length > 0) {
//       const result: ResultSet = await pool.query(CATEGORYQUERY.UPDATE_PATIENT, [...Object.values(patient), req.params.patientId]);
//       return res.status(Code.OK)
...
//     const result: ResultSet = await pool.query(CATEGORYQUERY.SELECT_PATIENT, [req.params.patientId]);
//     if (((result[0]) as Array<any>).length > 0) {
//       const result: ResultSet = await pool.query(CATEGORYQUERY.DELETE_PATIENT, [req.params.patientId]);
//       return res.status(Code.OK)
